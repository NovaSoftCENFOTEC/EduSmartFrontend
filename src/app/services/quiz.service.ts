...
export class QuizService extends BaseService<IQuiz> {
    ...
    getQuizWithQuestionsAndOptions(quizId: number) {
        return this.transformer.getQuizWithQuestionsAndOptions(
            (id: number) => this.getQuizById(id),
            quizId
        );
    }
    ...
    createQuizForStory(quiz: IQuiz, storyId: number) {
        ...
        const quizData = {
            ...
            dueDate: this.transformer.formatDateForBackend(quiz.dueDate),
    ...
    saveQuiz(item: IQuiz, storyId?: number) {
        ...
        this.add(item).subscribe({
            next: (response: IResponse<IQuiz>) => {
                ...
                storyId ? this.getQuizzesByStory(storyId) : this.getAll();
            },
            error: () => {
                this.alertService.displayAlert(
                    "error",
    ...
    update(item: IQuiz, storyId?: number) {
        ...
        const quizData = {
            ...item,
            dueDate: this.transformer.formatDateForBackend(item.dueDate)
        };
